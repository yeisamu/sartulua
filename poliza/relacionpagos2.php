...
jQuery.noConflict();
	...
	name:'id_contra'
		...
		,search:false,formatter:'currency',formatoptions:{decimalSeparator:'.',thousandsSeparator:',',decimalPlaces:0,prefix:'$'}},{";
...
$y++;
	...
	reimp = "<a id='inas' class='fm-button ui-state-default ui-corner-all fm-button-icon-left'  href='#' onClick=reimprime_cuota('imprime','"+trim(movil)+"','<?php echo $ani;?>');jQuery('#imprime').dialog('open');><span class='ui-icon ui-icon ui-icon-print'></span>Reimp</a>"; 
	    ...
	    editurl: 'gridpago.php?q=1'// this is dummy existing url 
		
		 });
		 
		 jQuery("#crud").jqGrid('navGrid','#pcrud',{edit:false,add:false,del:<?php if($permiso==1) { echo "true"; }else{ echo "false";}?>,search:false},{reloadAfterSubmit:true,closeAfterEdit : true,closeOnEscape:true},{closeOnEscape:true},{closeOnEscape:true},{sopt:['cn','bw','eq','ne','lt','gt','ew','le','ge'], odata : ['igual ', 'no igual a', 'menor que', 'menor o igual que','mayor que','mayor o igual a', 'empiece por','no empiece por','está en','no está en','termina por','no termina por','contiene','no contiene'],
