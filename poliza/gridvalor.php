...
$crudColumns1 =  array(
    'id'=>'id_valorp' 
	,'id_tipov'=>'valor_poliza.id_tipov'
    ,'tipov'=>'tipov'
	,'valorp'=>'valorp'
	,'vini'=>'vini'
	,'vc1'=>'vc1'
...
$crudTableName = 'valor_poliza';
$postConfig['id'] = 'id_valorp'; 
...
switch($postConfig['action']){
	case $crudConfig['read']:
		...
		/*query to count rows*/
		$sql='select count('.$postConfig['id'].') as numRows from valor_poliza ';
		...
		/*Run the data query*/
		$sql = 'select '.implode(',',$crudColumns1).'  from valor_poliza inner join tipov on valor_poliza.id_tipov=tipov.id_tipov';
	...
	$sql .= ' where id_valorp = '.$crudColumnValues['id'];
		if($DEBUGMODE == 1){$firephp->info($sql,'query');}
		mysql_query( $sql ) 
		or die($firephp->error('Couldn t execute query.'.mysql_error()));
		break;
	case $crudConfig['delete']:
		...
		$sql = 'Delete from '.$crudTableName.' where id_valorp = '.$crudColumnValues['id'];
		if($DEBUGMODE == 1){$firephp->info($sql,'query');}
		mysql_query( $sql ) 
		or die($firephp->error('Couldn t execute query.'.mysql_error()));
		break;
