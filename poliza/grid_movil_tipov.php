...
switch($postConfig['action']){
	case $crudConfig['read']:
		...
		/*query to count rows*/
		$sql='select count('.$postConfig['id'].') as numRows from tipo_taxi inner join tipov on tipo_taxi.id_tipov=tipov.id_tipov';
		...
		/*Run the data query*/
		$sql = 'select '.implode(',',$crudColumns).'  from tipo_taxi inner join tipov on tipo_taxi.id_tipov=tipov.id_tipov'.$wh;
