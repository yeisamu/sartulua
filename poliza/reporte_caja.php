...
$selmov=mysql_query("select * from (movi_contra left join (vehiculo inner join (tipo_taxi inner join tipov on tipo_taxi.id_tipov=tipov.id_tipov) on tipo_taxi.id_movil=vehiculo.id_movil) on movi_contra.id_movil=vehiculo.id_movil) where fecha_mov BETWEEN  '$fechaini' AND '$fechafinmov' order by fecha_mov asc");
...
<table width="100%" border="1" align="center"  class="ui-corner-all">
 ...
 <tr>
  ...
  <tr>
    <td colspan="2"><div align="center">RESUMEN DE CAJA </div></td>
    </tr>
  <tr>
    <td width="78%">TOTAL INGRESOS </td>
    <td width="22%"><div align="right"><strong>$ <?php echo number_format($toting,0,",",".")?></strong>
          </div>
    </div></td>
  </tr>
  <tr>
    <td>MAS SALDO ANTERIOR </td>
    <td><div align="right"><strong>$
      <?php   $fsaldo=aumenta_n_dia($fechaini,-1); $selsaldo=@mysql_query("SELECT `saldo` FROM `movi_contra` WHERE `fecha_mov` <='$fsaldo' order by `id_mov` desc limit 0,1"); $saldoant=@mysql_result($selsaldo,0,saldo); if($saldoant==0){ echo $saldoant=0;}else{	echo  number_format($saldoant,0,",",".");}?>
    </strong></div></td>
  </tr>
  <tr>
    <td>TOTAL</td>
    <td><div align="right"><strong>$ <?php echo  number_format($totent=$toting+$saldoant,0,",",".")?></strong></div></td>
  </tr>
  <tr>
    <td>MENOS SALIDAS DE CAJA </td>
    <td><div align="right"><strong>$ <?php echo number_format($totegre,0,",",".")?></strong></div></td>
  </tr>
  <tr>
    <td>SALDO EN CAJA A LA FECHA <?php echo $fechafinmov ?></td>
    <td><div align="right"><strong>$<?php echo  number_format($totent-$totegre,0,",",".") ?></strong></div></td>
  </tr>
</table>
</td>
   </tr>
