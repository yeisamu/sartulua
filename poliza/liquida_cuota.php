...
//echo $consuvalor1="select date_format(fini,'%Y-%m-%d') as fini,date_format(ffin,'%Y-%m-%d') as ffin,valorp,vini,vc1,vc2 from `valor_poliza` WHERE `id_tipov` = $tipo and year(fini)=$ini and year(ffin)=$fin  and grupo='$grupomov' ";
$consuvalor=mysql_query("select date_format(fini,'%Y-%m-%d') as fini,date_format(ffin,'%Y-%m-%d') as ffin,valorp,vini,vc1,vc2 from `valor_poliza` WHERE `id_tipov` = $tipo and year(fini)=$ini and year(ffin)=$fin  and grupo='$grupomov' ");
...
//echo ">1 select * from `detalle_contra` where id_movil='$idmovil' and periodo = '$per' and ncuota=$ncuotasya and grupo='$grupomov'";
...
//echo $filcuota['ncuota'];
// }
 
 if($filcuota['ncuota']==1){
 $fechainipol=$filcuota['fhasta'];//aumenta_n_mes($fini,1);//$fini;
 $fechafinpoli=aumenta_n_mes($filcuota['fhasta'],1);
...
<table width="100%" border="1" align="left" class="ui-corner-all ">
  <tr class="ui-corner-all ">
   <td colspan="4">
  ...
  <tr>
    <td colspan="4"><table width="100%" border="1" class="ui-corner-all">
      ...
      <tr class="ui-corner-all " >
        ...
        <td width="9%"><input name="fecha_ini" type="text" class="ui-corner-all " id="fecha_ini" value="<?php echo $fechainipol?>"  size="10" maxlength="10"  /></td>
        <td width="13%" class="ui-widget-header" >Hasta</td>
        <td width="20%"><input type="text" id="fecha_fin" name="fecha_fin" value="<?php echo $fechafinpoli?>" class="ui-corner-all "   size="10" maxlength="10" /></td>
        <td width="31%" class="ui-widget-header" >Saldo Poliza </td>
        <td width="12%"><?php echo $filacontra['saldo']?><input type="hidden" id="saldo" name="saldo" value="<?php echo $filacontra['saldo']?>" /></td>
      </tr>
      <tr class="ui-corner-all " >
        <td class="ui-widget-header" >Total Cuota </td>
        <td><input type="text" id="vcuota" name="vcuota" class="ui-corner-all " value="<?php echo $valcuota;?>"    size="8" maxlength="8" onChange="calculasaldo(<?php echo $valorpolizamovil?>);calculaffin(<?php echo $filacontra['saldo']?>,'<?php echo $ffin?>','<?php echo $fechafinpoli?>')" /></td>
        ...
        <td><input type="text" id="fecha_rc" name="fecha_rc" class="ui-corner-all "   size="10" maxlength="10" /><input type="hidden" id="ncontra" name="ncontra" value="<?php echo $ncontra?>" /><input type="hidden" id="idmovil" name="idmovil" value="<?php echo $idmovil?>" /><input type="hidden" id="period" name="period" value="<?php echo $per?>" /></td>
      </tr>
      <tr class="ui-corner-all " >
        <td colspan="2" class="ui-widget-header" ># Cuota </td>
        <td ><input class="ui-corner-all" type="text" id="ncuo" name="ncuo" value="<?php echo $ncuota?>"  size="3" /></td>
        <td colspan="2" class="Estilo4" ><div align="right" id="nsaldo">$ <?php echo number_format($nsaldo,"0","",".")?></div></td>
        <td class="ui-widget-header">Nuevo Saldo</td>
      </tr>
    </table></td>
    </tr>
  <tr>
