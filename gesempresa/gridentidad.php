...
$crudTableName = 'entidad_salud';
$postConfig['id'] = 'id_eps'; 
...
switch($postConfig['action']){
	case $crudConfig['read']:
		...
		/*query to count rows*/
		$sql='select count('.$postConfig['id'].') as numRows from entidad_salud ';
		...
		/*Run the data query*/
		$sql = 'select '.implode(',',$crudColumns).'  from entidad_salud';
	...
	case $crudConfig['update']:
		...
		$sql .= ' where id_eps = '.$crudColumnValues['id'];
		if($DEBUGMODE == 1){$firephp->info($sql,'query');}
		mysql_query( $sql ) 
		or die($firephp->error('Couldn t execute query.'.mysql_error()));
		break;
