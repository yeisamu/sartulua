...
<body>
<img src="../images/nuevas/impresionespresupuestos.jpg" alt="Impresión de Presupuestos"><br>
<br>

<form name="formul" method="post" action="presupuestos.php">
  <? if ($ini==1) {
     $error=0;
  ...
  <table width="80%" border="0" align="center" cellpadding="0" cellspacing="0" bordercolor="#0066FF">
    ...
    <tr> 
      ...
      <td colspan="2">
	  <? if ($ini==0) { ?>
	  <input name="codpresupuesto" type="text" id="albaran" size="8" maxlength="8">
	  <? } else { ?>
	  <input name="codpresupuesto" type="text" id="albaran" size="8" maxlength="8" value="<? echo $codpresupuesto; ?>">
	  <? } ?>
    </td></tr>
    <tr> 
...
<td class="primeralineaizquierda"><font size="2" face="Verdana, Arial, Helvetica, sans-serif">Ordenar 
      ...
      <td width="25%"><select name="campo" size="1">
          <option selected value="codpresupuesto">Cod. Presupuesto</option>
	  ...
	  <? }  
	  if ($ini==1) { 
	     if ($campo=="codpresupuesto") { ?>
		 <td width="25%"><select name="campo" size="1"><option selected value="codpresupuesto">Cod. Presupuesto</option>
          <option value="fecha">Fecha</option>
          <option value="nombre">Cliente</option></select></td>
		  <? } else { 
		  if ($campo=="fecha") { ?>
		  <td width="25%"><select name="campo" size="1"><option  value="codpresupuesto">Cod. Presupuesto</option>
    ...
    <tr> 
      <td colspan="3"><div align="center"> 
          <input type="submit" name="Submit" value="Buscar presupuestos">
...
<p>
 <? if ($ini==1) {
     $consulta = "Select * from presupuestos,clientes";
     $consulta2 = "where presupuestos.codcliente=clientes.codcliente and ";
	 include ("../cascadapresupuestos.php");
	 $consulta = $consulta . " " . "order by ".$campo." ".$ordenacion." ";
     $resultado = mysql_query($consulta, $conexion);
     $filas=mysql_num_rows($resultado);
	 if (empty($numi)) { $numi=0; }
	 print "<center><font size=2 face='Verdana, Arial, Helvetica, sans-serif'>Número de presupuestos " . $filas . "</font></center>";
...
<table align="center">
  <tr> 
    <td width="10%" class="primeralinea">Cod.Presupuesto</td>
    <td width="20%" class="primeralinea">Cliente</td>
    <td width="7%" class="primeralinea">Fecha</td>
    <td width="9%" class="primeralinea">Verpresupuesto</td>
  </tr>
  ...
  <tr> 
    ...
    <td class="segundalinea"><font color="#FF0000"><b><? echo $lafila["codpresupuesto"]; ?></b></font></td>
    <td class="segundalineaizquierda"><font color="#FF0000"><? echo $lafila1["nombre"]; ?></font></td>
    <td class="segundalinea"><? echo $fech; ?></td>
    <td>
    <form name="form2" method="post" action="vista_presupuesto.php">
	<center>
      <input type="submit" name="Submit2" value="Imprimir presupuesto"></center>
      <font size="1"></td></font> 
      <input type="hidden" name="codpresupuesto" value="<? echo $lafila["codpresupuesto"]; ?>">
    </form>
  </tr>
  <? } ?>
</table> 
  ...
  while ($i<$enlaces) { ?>
      <a href="presupuestos.php?fecha1=<? echo reconversion($fecha1); ?>&fecha2=<? echo reconversion($fecha2); ?>&codigo=<? echo $codigo; ?>&ini=1&numi=<? echo $i; ?>"><? echo $j; ?></a>
 <? $j++; 
 $i=$i+10; }
 }
