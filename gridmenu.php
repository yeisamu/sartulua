...
$crudColumns1 =  array(
    'id'=>'opcion'
	,'id_grupo'=>'acc_grupo.id_grupo'
    ,'grupo'=>'grupo' 
	,'path'=>'path' 
	//,'id_opcion'=>'acc_opcion.id_opcion'
//	,'opcion'=>'opcion'
//	,'permiso'=>'permiso'
	//,'id_conductor'=>'conductor.codigo'
//	,'nombre'=>'conductor.nombre1'
//	,'apellido'=>'conductor.apellido1'
/*	,'categoria'=>'categoria'
	,'fecha_vence'=>'fecha_vence'
	,vehiculo.id_movil,vehiculo.placa,conductor.codigo,concat(conductor.nombre1,' ',conductor.nombre2) as nombre,concat(conductor.apellido1,' ',conductor.apellido2) as apellido*/
//	,'apellido1'=>'apellido1'
//	,'apellido2'=>'apellido2'
	//,'direccion'=>'direccion'
//	,'telefono'=>'telefono'
...
$crudTableName = 'acc_opcion';
$postConfig['id'] = 'opcion'; 
...
switch($postConfig['action']){
	case $crudConfig['read']:
		...
		/*query to count rows  */
		$sql='select count('.$postConfig['id'].') as numRows from (`acc_opcion` inner join acc_grupo on acc_opcion.`id_grupo`=acc_grupo.`id_grupo`) inner join acc_permiso on acc_opcion.`id_opcion` =acc_permiso .`id_opcion`  where id_usr='.$id.' AND permiso=1 And visible=1';
		...
		/*Run the data query */
		$sql = 'select '.implode(',',$crudColumns1).' FROM (`acc_opcion` inner join acc_grupo on acc_opcion.`id_grupo`=acc_grupo.`id_grupo`) inner join acc_permiso on acc_opcion.`id_opcion` =acc_permiso .`id_opcion` WHERE  id_usr='.$id.' AND permiso=1 And visible=1' ;
		if($postConfig['search'] == 'true'){
			$sql .= ' AND '. $postConfig['searchField'] . ' ' . fnSearchCondition($_REQUEST['searchOper'], $postConfig['searchStr']);
		}
	...
	case $crudConfig['update']:
		...
		$sql = 'update acc_permiso set permiso='.$crudColumnValues['permiso'].' where id_permiso='.$crudColumnValues['id'];
	...
	case $crudConfig['delete']:
		...
		$sql = 'Delete from '.$crudTableName. ' where `id_permiso` ='.$crudColumnValues['id'];
		if($DEBUGMODE == 1){$firephp->info($sql,'query');}
		mysql_query( $sql ) 
		or die($firephp->error('Couldn t execute query.'.mysql_error()));
		break;
