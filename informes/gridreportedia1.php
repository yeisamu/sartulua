...
for($dia=1; $dia<=$dias;$dia++){
    //$consulta = $consulta.", sum(if(day(fecha_reg)=".$dia.",1,0)) as '".$dia."'"; 
	 ...
	 $consulta = $consulta." , sum(if(if(time(fecha_reg)<'23:59:59' and time(fecha_reg)>'05:59:59',day(fecha_reg),day(DATE_SUB(fecha_reg,INTERVAL 1 day)))=".$dia.",1,0))
	 as '".$dia."'"; 
...
$consulta = $consulta.", count(fecha_reg) as total,estado,case estado when 0 then 'Pendiente' when 1 then 'Asignado' when 2 then 'Descartado' end as est from servicio_h inner join linea_atencion on servicio_h.linea=linea_atencion.linea  where month(fecha_reg)= ".$mes." and year(fecha_reg)= ".$ano;
return $consulta;
}
...
// $var="SELECT COUNT(*) AS count from servicio where 1 ".$wh.$wd;
 ...
 //$row['count'];
        ...
        if ($start<0) $start = 0;  $SQL = consulta_mes($tmes,$anio).$wd." group by servicio_h.linea order by ".$sidx." ". $sord;
