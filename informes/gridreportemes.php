...
for($mes=1; $mes<=12;$mes++){
     $inicio=$ano.'-'.$mes.'-01 05:59:59';
	   $mes2=$mes+1;
	  $fin=$ano.'-'.$mes2.'-01 05:59:59';
    $consulta = $consulta.", sum(if(fecha_reg > '$inicio' and  fecha_reg<'$fin' ,1,0)) as '".$meses[$mes - 1]."'"; 
}
$consulta = $consulta.", count(fecha_reg) as total,servicio_h.estado,case servicio_h.estado when 0 then 'Pendiente' when 1 then 'Asignado' when 2 then 'Descartado' end as est from servicio_h inner join linea_atencion on servicio_h.linea=linea_atencion.linea where year(fecha_reg)= ".$ano;
return $consulta;
}
