...
$crudTableName = 'acc_usuario';
$postConfig['id'] = 'id_usr'; 
...
switch($postConfig['action']){
	case $crudConfig['read']:
		...
		/*query to count rows*/
		$sql='select count('.$postConfig['id'].') as numRows from acc_usuario ';
		...
		/*Run the data query*/
		$sql = 'select '.implode(',',$crudColumns).'  from acc_usuario';
	...
	case $crudConfig['update']:
		...
		$sql = 'update '.$crudTableName." set login='$login',clave='$pass',admin='$admin'";
		...
		$sql .= ' where id_usr = '.$crudColumnValues['id'];
		if($DEBUGMODE == 1){$firephp->info($sql,'query');}
		mysql_query( $sql ) 
		or die($firephp->error('Couldn t execute query.'.mysql_error()));
		break;
	case $crudConfig['delete']:
		...
		$sql = 'Delete from '.$crudTableName.' where id_usr = '.$crudColumnValues['id'];
		if($DEBUGMODE == 1){$firephp->info($sql,'query');}
		mysql_query( $sql ) 
		or die($firephp->error('Couldn t execute query.'.mysql_error()));
		break;
