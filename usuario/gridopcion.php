...
switch($postConfig['action']){
	case $crudConfig['read']:
		...
		/*query to count rows*/
		$sql='select count('.$postConfig['id'].') as numRows from acc_opcion ';
		...
		/*Run the data query    '.implode(',',$crudColumns).'*/
		$sql = "select id_opcion,opcion,descripcion,acc_grupo.id_grupo,grupo,path,visible,case visible when 0 then 'No Visible' else 'Visible' end as visible2  from acc_opcion inner join acc_grupo on acc_opcion.id_grupo=acc_grupo.id_grupo";
	...
	case $crudConfig['update']:
		...
		$sql .= ' where id_opcion = '.$crudColumnValues['id'];
		if($DEBUGMODE == 1){$firephp->info($sql,'query');}
		mysql_query( $sql ) 
		or die($firephp->error('Couldn t execute query.'.mysql_error()));
		break;
	case $crudConfig['delete']:
		...
		$sql = 'Delete from '.$crudTableName.' where id_opcion = '.$crudColumnValues['id'];
		if($DEBUGMODE == 1){$firephp->info($sql,'query');}
		mysql_query( $sql ) 
		or die($firephp->error('Couldn t execute query.'.mysql_error()));
		break;
