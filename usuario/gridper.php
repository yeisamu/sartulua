...
$crudTableName = 'acc_permiso';
$postConfig['id'] = 'id_permiso'; 
...
switch($postConfig['action']){
	case $crudConfig['read']:
		...
		/*query to count rows  */
		$sql='select count('.$postConfig['id'].') as numRows from `acc_permiso` where id_usr='.$id;
		...
		/*Run the data query */
		$sql = 'select '.implode(',',$crudColumns1).' FROM (`acc_permiso` inner join acc_opcion on `acc_permiso`.`id_opcion`=acc_opcion.`id_opcion`) WHERE  id_usr='.$id;
