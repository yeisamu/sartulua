...
$crudTableName = 'acc_grupo';
$postConfig['id'] = 'id_grupo'; 
...
switch($postConfig['action']){
	case $crudConfig['read']:
		...
		/*query to count rows*/
		$sql='select count('.$postConfig['id'].') as numRows from acc_grupo ';
		...
		/*Run the data query*/
		$sql = 'select '.implode(',',$crudColumns).'  from acc_grupo';
	...
	case $crudConfig['update']:
		...
		$sql .= ' where id_grupo = '.$crudColumnValues['id'];
		if($DEBUGMODE == 1){$firephp->info($sql,'query');}
		mysql_query( $sql ) 
		or die($firephp->error('Couldn t execute query.'.mysql_error()));
		break;
	case $crudConfig['delete']:
		...
		$sql = 'Delete from '.$crudTableName.' where id_grupo = '.$crudColumnValues['id'];
		if($DEBUGMODE == 1){$firephp->info($sql,'query');}
		mysql_query( $sql ) 
		or die($firephp->error('Couldn t execute query.'.mysql_error()));
		break;
