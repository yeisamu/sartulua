...
$crudColumns =  array(
	...
	,'nombre'=>"concat_ws(' ',nombre,apellidos) as nombre" 
	,'placa'=>'placa'
	 ,'clase'=>'clase' 
	,'marca'=>'marca'
...
switch($postConfig['action']){
	case $crudConfig['read']:
		...
		/*query to count rows*/
		$sql='select count('.$postConfig['id'].') as numRows from vehiculo';
		...
		/*Run the data query*/
		$sql = 'select '.implode(',',$crudColumns).'  from (vehiculo inner join marca on vehiculo.id_marca=marca.id_marca) left join propietario on vehiculo.id_prop=propietario.id_prop';
		...
		while($row = mysql_fetch_array($result,MYSQL_ASSOC)) { 
			...
			$o->rows[$i]['cell']=array($row[id_movil],$row[id_prop],utf8_encode("$row[nombre]"),$row[placa],$row[clase],$row[marca],$row[referencia],$row[tipo],$row[motor],$row[serie],$row[color],$row[modelo],$row[grupo]);
			$i++; 
		}
