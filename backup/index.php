...
if ($CONF['sql_db'] && !@mysql_select_db($CONF['sql_db'])) echo PMBP_addOutput(C_WRONG_DB,"red_left");
...
if (function_exists("phpversion")) {
    ...
    if ($phpvers>=4.3) $php=$tmp; else $php=PMBP_addOutput($tmp,"red_left");
} else {
    $php=PMBP_I_NO_RES;
}
if (!($memory_limit=ini_get("memory_limit"))) $memory_limit=PMBP_I_NO_RES;
if (function_exists("ftp_connect")) $ftp=F_YES; else $ftp=PMBP_addOutput(F_NO,"red_left");
if (function_exists("mail")) $mail=F_YES; else $mail=PMBP_addOutput(F_NO,"red_left");
