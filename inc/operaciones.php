...
function deme_now(){
 $consultadeta = "SELECT now() as ahora";
 $resuldeta=mysql_query($consultadeta);  
	$detalle=mysql_fetch_array($resuldeta);
	$fecha_vendoc=$detalle['ahora'];	  
	return 	$fecha_vendoc;
}
...
$dias_planilla=deme_plazo_planilla();
 ...
 //$calculo1= strtotime("7 hours", $calculo);
	   $nfechas= strtotime("8 hours", $fechaCom);
...
function aumenta_dias($fecha,$can_dias){
 ...
 $calculo1= strtotime("8 hours", $calculo);
...
function aumenta_n_mes($fecha,$can_dias){
 $fechaComparacion = strtotime($fecha,$can_dias);
 //$can_dias=1;
 $calculo1= strtotime("$can_dias month", $fechaComparacion);
 //$calculo1= strtotime("7 hours", $calculo);
 $fecha_ok=date("Y-m-d", $calculo1);
 return $fecha_ok;
} 
...
$mes=date('Y-m');
$consulta=mysql_query("select count(id_planilla) as total from planilla inner join tarjeta_control on planilla.id_tarjeta=tarjeta_control.id_tarjeta where planilla.estado <= 2 and date_format(`fecha_inicio`,'%Y-%m')= '$mes' and id_movil=$id_movil ");
