...
INNER JOIN conductor b ON a.`id_conductor` = b.`id_conductor` WHERE estado = 1  and `fecha_plazo_a` <DATE_FORMAT(now(),'%Y/%m/%d %H:%i') ".$wh.$wd);
  ...
  $count = $row['count'];
        ...
        if ($start<0) $start = 0;
   $SQL = "SELECT b.`id_conductor` ,`id_tarjeta`,`tarjeta`, codigo,nombre1,nombre2,apellido1,apellido2,a.`id_movil` as id_movil, fecha_vigencia,fecha_plazo_a,if(`fecha_plazo_a`<DATE_FORMAT(now(),'%Y/%m/%d %H:%i'),'Suspendido','Permitido') as servicio ,est_ant,if(`fecha_plazo_a`<DATE_FORMAT(now(),'%Y/%m/%d %H:%i'),'0','1') as est_new,grupo FROM (tarjeta_control a inner join vehiculo on a.id_movil=vehiculo.id_movil)
INNER JOIN conductor b ON a.`id_conductor` = b.`id_conductor` WHERE estado = 1 having servicio='Suspendido'  ".$wh.$wd." ORDER BY ".$sidx." ". $sord." LIMIT ".$start." , ".$limit; 
	...
	$responce->rows[$i]['cell']=array($row[id_tarjeta],$row[tarjeta],$row[codigo],$row[nombre1],$row[nombre2],$row[apellido1],$row[apellido2],$row[id_movil],$row[fecha_vigencia],$row[id_conductor],$row[fecha_plazo_a],$row[servicio],$row[est_ant],$row[est_new],$row[grupo],$fila[planillas_mes],$totdiamovil); $i++; }
	//echo $SQL;
