...
//   $result = mysql_query("SELECT COUNT(*) AS count from servicio".$wh.$wd);
...
//        if ($start<0) $start = 0;
 $id = $_GET['id']; 
  // $SQL = "SELECT distinct(month(`fecha_reg`)) as mes FROM `servicio` WHERE year(fecha_reg)=".$id." ORDER BY mes";
 $SQL = "SELECT distinct(month(`fecha_reg`)) as mes FROM `servicio_h` WHERE year(fecha_reg)='".$id."' ORDER BY mes "; 
      $result = mysql_query( $SQL ) or die("Couldntexecutequery.".mysql_error());
	   $responce->page = $page;
	$responce->total = $total_pages; 
	$responce->records = $count; $i=0; 
	while($row = mysql_fetch_array($result,MYSQL_ASSOC)) {
	$meses = array(1=>'ENERO',2=> 'FEBRERO',3=> 'MARZO',4=> 'ABRIL',5=> 'MAYO',6=> 'JUNIO',7=> 'JULIO',8=> 'AGOSTO', 9=>'SEPTIEMBRE',10=> 'OCTUBRE',"11"=> 'NOVIEMBRE',12=> 'DICIEMBRE'); 
	 $path='reposerviciomes.php';
	$mesac=$row[mes];
	$mes=$meses[$row[mes]];
	$responce->rows[$i]['id']=$row[mes]; 
	$responce->rows[$i]['cell']=array($mes,$path); $i++; }
	//echo $SQL;
	...
	case 2:
		...
		$id=$_REQUEST['id'];
		$sqls = 'Delete from servicio_h where id_nov_serv='.$id;
