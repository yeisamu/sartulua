...
switch($postConfig['action']){
	case $crudConfig['read']:
	   ...
	   $sql='select count('.$postConfig['id'].") as numRows from servicio_h where estado>=1 and  year(fecha_reg)= ".$ano.$wd;
		...
		$intStart = (($intPage-1) * $intLimit);
		 ...
		 ///*****************
		 	$sql = 'select '.implode(',',$crudColumns1)." from servicio_h where estado>=1  and  year(fecha_reg)= ".$ano;
	...
	$o->rows[$i]['cell']=array($row[id_ser],$row[telefono],$row[linea],$row[direccion],$row[detalle_serv],$row[fecha_reg],$row[id_movil],$row[id_movil2],$row[nombres],$row[usuario],$row[estado],$row[observacion]); $i++; }
		break;
