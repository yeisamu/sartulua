...
switch($postConfig['action']){
	case $crudConfig['read']:
     ...
     $start=0;   
		...
		//$sql="select  HIGH_PRIORITY count(".$postConfig['id'].") as numRows from servicio where estado=0 ";
		//$sql.=$wd;
			 	$sql = 'select  HIGH_PRIORITY '.implode(',',$crudColumns1).",if (MINUTE(TIMEDIFF(now(),servicio.fecha_reg))<=5,1,if(MINUTE(TIMEDIFF(now(),servicio.fecha_reg))<=8,2,3)) as color   from servicio where servicio.estado=0  ";
...
$vari=" ";
	$o->rows[$i]['cell']=array($vari,$row[id_ser],$row[linea],$row[telefono],utf8_encode("$row[direccion]"),utf8_encode("$row[detalle_serv]"),$row[fecha_reg],$row[id_movil],$row[color]); $i++; }
		break;
...
//	$sql .= implode(',',$crudColumnValues);
	...
	$usuario=$_SESSION['login'];
		...
		$sql = 'update '.$crudTableName." set  `direccion`='$direccion',`telefono`='$tel',`id_movil`='$id_movil',placa='$placa', `fecha_asig`=now(), `id_tarjeta`='$id_tarjeta', `tarjeta`='$tarjeta', `id_conductor`='$id_conductor', `codigo`='$codigo', `nombres`='$nombres', `estado`=$estado ";
	 ...
	 $sql .= ' where id_ser = '.$crudColumnValues['id'];
