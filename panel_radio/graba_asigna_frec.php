...
$tarjeta=$filad['tarjeta'];
 ...
 $compro="insert into `comp_servicio` (`fecha`, `usuario`, `id_movil`, `placa`, `id_conductor`, `codigo`, `nombres`, `id_tarjeta`, `tarjeta`, `id_tran`) values ('$fechact','$usuario','$id_movil','$placa',$id_conductor,'$codigo','$nombres',$id_tarjeta,'$tarjeta',12)";
 $frec="insert into `frecuencia` (`id_movil`, `id_tarjeta`, `fecha`) values('$id_movil',$id_tarjeta,'$fechact')";

$compro=mysql_query("insert into `comp_servicio` (`fecha`, `usuario`, `id_movil`, `placa`, `id_conductor`, `codigo`, `nombres`, `id_tarjeta`, `tarjeta`, `id_tran`) values ('$fechact','$usuario','$id_movil','$placa',$id_conductor,'$codigo','$nombres',$id_tarjeta,'$tarjeta',12)");

$frec=mysql_query("insert into `frecuencia` (`id_movil`, `id_tarjeta`, `fecha`) values('$id_movil',$id_tarjeta,'$fechact')");
...
<script language='javascript'>
	    jQuery('#crudfre').trigger('reloadGrid');jQuery('#crudac').trigger('reloadGrid');jQuery('#frecuencia').dialog( 'close' );
	   </script>";
	}	
//<div class='ui-state-error' id='errorgraba' title='Grabar Reporte de Frecuencia'>
//		<span class='ui-icon ui-icon-alert' style='float:left; margin:0 7px 50px 0;'></span>
//	   Reporde de Frecuencia del Movil $id_movil Grabado con exito </div>	jQuery('#errorgraba').dialog({modal: true,buttons: {Ok: function() {
