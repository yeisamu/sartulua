...
$crudColumns =  array(
	...
	,'codigo'=>'codigo'
	,'nombres'=>"concat(nombre1,' ',nombre2,' ',apellido1,' ',apellido2)"
...
switch($postConfig['action']){
	case $crudConfig['read']:
		...
		/*query to count rows*/
		$sql='select count('.$postConfig['id'].') as numRows from servicio ';
		...
		/*Run the data query*/
		$sql = 'select '.implode(',',$crudColumns).'  from servicio';
	...
	case $crudConfig['delete']:
		...
		$sql = 'Delete from '.$crudTableName.' where id_ser = '.$crudColumnValues['id'];
		if($DEBUGMODE == 1){$firephp->info($sql,'query');}
		mysql_query( $sql ) 
		or die($firephp->error('Couldn t execute query.'.mysql_error()));
		break;
