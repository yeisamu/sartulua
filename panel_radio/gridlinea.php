...
$crudColumns =  array(
    'id'=>'id_linea' 
	,'linea'=>'linea'
	,'id_tipo_linea'=>'linea_atencion.id_tipo_linea'
...
$crudTableName = 'linea_atencion';
$postConfig['id'] = 'id_linea'; 
...
switch($postConfig['action']){
	case $crudConfig['read']:
		...
		/*query to count rows*/
		$sql='select count('.$postConfig['id'].') as numRows from linea_atencion ';
		...
		/*Run the data query*/
		$sql = 'select '.implode(',',$crudColumns).'  from linea_atencion inner join tipo_linea on tipo_linea.id_tipo_linea=linea_atencion.id_tipo_linea';
	...
	case $crudConfig['update']:
		...
		$sql .= ' where id_linea = '.$crudColumnValues['id'];
		if($DEBUGMODE == 1){$firephp->info($sql,'query');}
		mysql_query( $sql ) 
		or die($firephp->error('Couldn t execute query.'.mysql_error()));
		break;
	case $crudConfig['delete']:
		...
		$sql = 'Delete from '.$crudTableName.' where id_linea = '.$crudColumnValues['id'];
		if($DEBUGMODE == 1){$firephp->info($sql,'query');}
		mysql_query( $sql ) 
		or die($firephp->error('Couldn t execute query.'.mysql_error()));
		break;
