...
switch($postConfig['action']){
	case $crudConfig['read']:
		...
		/*query to count rows*/
		$sql="select count(".$postConfig['id'].") as numRows from ".$crudTableName." where 1 ";
		$sql.=$wd;
...
OR ADDDATE( DATE_FORMAT( concat( vehiculo.pago_hasta, ' 11:59:59' ) , '%Y/%m/%d %H:%i' ) , 37 ) < DATE_FORMAT( now( ) , '%Y/%m/%d %H:%i' )*/
		 ...
		 ///*****************
		 	$sql = 'select '.implode(',',$crudColumns)."  from ".$crudTableName." where 1 ";
	...
	$o->rows[$i]['cell']=array($row[id_msj],$row[id_movil],utf8_encode("$row[msj]"),$row[estado],$row[fecha_reg]); $i++; }
	//echo $SQL;
