...
$crudTableName = 'directorio';
$postConfig['id'] = 'telefono'; 
...
switch($postConfig['action']){
	case $crudConfig['read']:
		...
		/*query to count rows*/
		$sql='select count('.$postConfig['id'].') as numRows from directorio ';
		...
		/*Run the data query*/
		$sql = 'select '.implode(',',$crudColumns).'  from directorio';
	...
	case $crudConfig['create']:
		...
		$sql = 'insert into '.$crudTableName.'(`telefono`, `direccion`';
	...
	case $crudConfig['update']:
		...
		$sql = 'update '.$crudTableName." set telefono='$tel',direccion='$dir'";
		...
		$sql .= ' where telefono = '.$crudColumnValues['id'];
		if($DEBUGMODE == 1){$firephp->info($sql,'query');}
		mysql_query( $sql ) 
		or die($firephp->error('Couldn t execute query.'.mysql_error()));
		break;
	case $crudConfig['delete']:
		...
		$sql = 'Delete from '.$crudTableName.' where telefono = '.$crudColumnValues['id'];
		if($DEBUGMODE == 1){$firephp->info($sql,'query');}
		mysql_query( $sql ) 
		or die($firephp->error('Couldn t execute query.'.mysql_error()));
		break;
