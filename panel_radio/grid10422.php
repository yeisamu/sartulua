...
switch($postConfig['action']){
	case $crudConfig['read']:
		...
		/*query to count rows*/
		$sql='select count('.$postConfig['id'].') as numRows from accidente ';
		...
		/*Run the data query*/
		$sql = 'select '.implode(',',$crudColumns).'  from accidente';
...
$id_tarjeta=$datotc['id_tarjeta'];
		...
		$sql = 'insert into accidente (fecha,usuario,fecha_inc,dir_inc,id_tarjeta,tarjeta,codigo,nombres,id_movil,placa,id_tipo_a,tipo_accidente,placa_otro,info_otro,conduce_prop,ambulancia,lesionado,transito,tras_lesionado,entidad_lesionado';
		...
		$sql .= ")VALUES(now(),'$usuario','$fecha_inc','$diracc','$id_tarjeta','$tarjeta','$codigo','$nombres','$id_movil','$placa','$id_tipo_a','$tipo_acc','$placa_otro','$info_otro','$prop','$amb','$les','$transito','$tras','$entidad')";
...
$id_tarjeta=$datotc['id_tarjeta'];
	...
	echo	$sql .= "fecha=now(),usuario='$usuario',fecha_inc='$fecha_inc',dir_inc='$diracc',id_tarjeta=$id_tarjeta,tarjeta='$tarjeta',codigo='$codigo',nombres='$nombres',id_movil='$id_movil',placa='$placa',id_tipo_a=$id_tipo_a,tipo_accidente='$tipo_acc',placa_otro='$placa_otro',info_otro='$info_otro',conduce_prop='$prop',ambulancia='$amb',lesionado='$les',transito='$transito',tras_lesionado='$tras',entidad_lesionado='$entidad' where id_acc = ".$crudColumnValues['id'];
		if($DEBUGMODE == 1){$firephp->info($sql,'query');}
		mysql_query( $sql ) 
