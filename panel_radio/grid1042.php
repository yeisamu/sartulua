...
switch($postConfig['action']){
	case $crudConfig['read']:
		...
		/*query to count rows*/
		$sql='select count('.$postConfig['id'].') as numRows from accidente where estado=0';
		...
		/*Run the data query*/
		$sql = 'select '.implode(',',$crudColumns).'  from accidente where estado=0';
...
$id_tarjeta=$datotc['id_tarjeta'];
		...
		$sql .= ")VALUES(now(),'$usuario','$fecha_inc','$diracc','$id_tarjeta','$tarjeta','$codigo','$nombres','$id_movil','$placa','$id_tipo_a','$tipo_acc','$placa_otro','$info_otro','$prop','$amb','$les','$transito','$tras','$entidad','$fecha_asis','$danos_mat','$heridos','$conciliacion','$croquis','$juez_gtia','$rep_aseg','$observacion')";
