...
jQuery("#crudsasig").jqGrid({ url:'gridserv_asig.php',
	 datatype: "json", 
	  mtype: 'GET',
	  colNames:['','Telefono','Direccion','Movil','Movil','Usuario','Accion']
	  , colModel :[
   ...
   // viewrecords: true,
	...
	loadComplete:function(){
		...
		setTimeout(function(){jQuery('#crudsasig').trigger('reloadGrid')},100000)
...
jQuery("#crudradio").jqGrid({ url:'gridpto_radio.php?q=1',
   ...
   // viewrecords: true,
	...
	loadComplete:function(){
		...
		setTimeout(function(){jQuery('#crudradio').trigger('reloadGrid')},100000)
	 ...
	 if(id && id!==lastsel){ rowid = id;jQuery('#crudradio').jqGrid('restoreRow',lastsel); jQuery('#crudradio').jqGrid('editRow',id,true); jQuery("#crudradio").jqGrid('editCell',1,3,true);lastsel=id; }
   /*   if(id && id!==lastSel){ 
	   ...
	   rowid = id;
        ...
        // lastSel=id; 
		 ...
		 jQuery('#crudradio').jqGrid('restoreRow',lastsel);
		 jQuery('#crudradio').jqGrid('editRow',id,true);
		 lastsel=id;
      } */
	  // jQuery("#crudradio").jqGrid('editCell',2,3,true);
	/*  var ids = jQuery("#crudradio").getDataIDs();
		for(var i=0;i<ids.length;i++){
			var cl = ids[i];
		var rowData = jQuery(this).getRowData(cl);
	...
	//  jQuery('#id_servicio').val();
	 // jQuery('#id_servicio').val();
	    ...
	    editurl: 'gridpto_radio.php'// this is dummy existing url 
		 ...
		 jQuery("#crudradio").jqGrid('navGrid','#pcrudradio',{edit:false,add:true,del:false,view:false,search:false,reload:false},{reloadAfterSubmit:true,closeAfterEdit :true,closeOnEscape:true},{reloadAfterSubmit:true,closeOnEscape:true,closeAfterAdd : true},{closeOnEscape:true});
...
jQuery('#accidente').dialog({autoOpen: false, modal:true,width:750,height:400,});	
jQuery('#asigna_movil').dialog({autoOpen: false, modal:true,width:350,height:300,});	
...
<div id="Layer8">
  <table width="609" border="1" class="ui-corner-all">
  <tr class="ui-jqgrid-titlebar ui-widget-header ui-corner-top ">
    <td width="144" style="font-size:14px"  >Puntos</td>
    <td width="251"  style="font-size:14px" >Celulares</td>
	<td width="192" style="font-size:14px" >Fijo</td>
  </tr>
  <tr class="ui-corner-all ">
    <td><select id="pto_radio" name="pto_radio" onchange="carga_pto(this.value);dir_pto(this.value)" class="ui-corner-all" >
	<option value="" selected="selected">Seleccione Pto</option>
	<?php
	$link=conectarse();
	$conpto=mysql_query("select * from linea_atencion where id_tipo_linea=3");
  ...
  <tr class="ui-widget-header ui-corner-all">
    <td style="font-size:14px" >Tel&eacute;fono</td>
    <td style="font-size:14px" >Direcci&oacute;n</td>
	<td style="font-size:14px" >Detalle del Servicio </td>
  </tr>
  <tr>
    <td><input type="text" id="tele" name="tele" class="ui-corner-all" /></td>
    <td><input name="direcci" type="text" class="ui-corner-all" id="direcci" size="30" maxlength="50" /></td>
	<td><textarea name="detall" rows="1" class="ui-corner-all" id="detall"></textarea>
	<input type="hidden" id="linea_at" name="linea_at" value="Fijo"/></td>
  </tr>
   <tr>
    <td colspan="2"><div align="center">
     <button  type="button"  class="ui-corner-all "  onclick="grabar_ser('graba_ser')"  accesskey="g" /><u>G</u>rabar</button>
    </div></td>
	<td style="font-size:14px" colspan="1" id="linea_servi" >Fijo</td>
  </tr>
</table>
