...
$crudTableName = 'tipo_linea';
$postConfig['id'] = 'id_tipo_linea'; 
...
switch($postConfig['action']){
	case $crudConfig['read']:
		...
		/*query to count rows*/
		$sql='select count('.$postConfig['id'].') as numRows from tipo_linea ';
		...
		/*Run the data query*/
		$sql = 'select '.implode(',',$crudColumns).'  from tipo_linea';
	...
	case $crudConfig['update']:
		...
		$sql .= ' where id_tipo_linea = '.$crudColumnValues['id'];
		if($DEBUGMODE == 1){$firephp->info($sql,'query');}
		mysql_query( $sql ) 
		or die($firephp->error('Couldn t execute query.'.mysql_error()));
		break;
	case $crudConfig['delete']:
		...
		$sql = 'Delete from '.$crudTableName.' where id_tipo_linea = '.$crudColumnValues['id'];
		if($DEBUGMODE == 1){$firephp->info($sql,'query');}
		mysql_query( $sql ) 
		or die($firephp->error('Couldn t execute query.'.mysql_error()));
		break;
