...
switch($postConfig['action']){
	case $crudConfig['read']:
		...
		/*query to count rows*/
		$sql="select count(".$postConfig['id'].") as numRows from servicio where estado=0 ";
		$sql.=$wd;
...
OR ADDDATE( DATE_FORMAT( concat( vehiculo.pago_hasta, ' 11:59:59' ) , '%Y/%m/%d %H:%i' ) , 37 ) < DATE_FORMAT( now( ) , '%Y/%m/%d %H:%i' )*/
		 ...
		 ///*****************
		 	$sql = 'select '.implode(',',$crudColumns1).",if (MINUTE(TIMEDIFF(now(),fecha_reg))<=5,1,if(MINUTE(TIMEDIFF(now(),fecha_reg))<=8,2,3)) as color   from servicio where estado=0  ";
	...
	$o->rows[$i]['cell']=array($row[id_ser],$row[linea],$row[telefono],$row[direccion],$row[detalle_serv],$row[fecha_reg],$row[id_movil],$row[color]); $i++; }
		break;
