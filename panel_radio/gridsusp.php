...
switch($postConfig['action']){
	case $crudConfig['read']:
		...
		/*query to count rows*/
		$sql="select count(".$postConfig['id'].") as numRows from suspension left join vehiculo on suspension.id_movil=vehiculo.id_movil where DATE_FORMAT(f_inicio,'%Y-%m-%d %H:%i' ) < DATE_FORMAT(now(), '%Y-%m-%d %H:%i' ) AND DATE_FORMAT(f_fin,'%Y-%m-%d %H:%i' ) > DATE_FORMAT(now(), '%Y-%m-%d %H:%i' ) ";
		$sql.=$wd;
...
OR ADDDATE( DATE_FORMAT( concat( vehiculo.pago_hasta, ' 11:59:59' ) , '%Y/%m/%d %H:%i' ) , 37 ) < DATE_FORMAT( now( ) , '%Y/%m/%d %H:%i' )*/
		 ...
		 ///*****************
		 	$sql = 'select '.implode(',',$crudColumns1)."  from suspension left join vehiculo on suspension.id_movil=vehiculo.id_movil where DATE_FORMAT(f_inicio,'%Y-%m-%d %H:%i' ) < DATE_FORMAT(now(), '%Y-%m-%d %H:%i' ) AND DATE_FORMAT(f_fin,'%Y-%m-%d %H:%i' ) > DATE_FORMAT(now(), '%Y-%m-%d %H:%i' ) ";
...
//	$sql .= implode(',',$crudColumnValues);
	...
	$usuario=$_SESSION['login'];
		...
		$sql = 'update '.$crudTableName." set  `id_movil`='$id_movil', `f_inicio`='$f_ini', `f_fin`='$ffin', `descripcion`='$descripcion', `usuario`='$usuario'";
