  ...
  /*  $.jgrid.no_legacy_api = true;
	...
	},
	{
	  ...
	  name:'est_new'
		...
		,align:'center'	
		,hidden:true
		},{name:'accion',index:'accion',align:'left', width:120,sortable:false,search:false	
...
jQuery("#crudtc").jqGrid({ 
	 ...
	 datatype: "json", 
	  mtype: 'GET',
	   colNames:['Codigo','','Nro Planilla','Fecha Elab','Movil','Conductor','Destino','Fecha Sal','Fecha Ret','Fecha Entrega','Estado','Observacion','Operaciones','','','']
	, colModel :[
	{name:'id_planilla'
		,index:'id_planilla'
		,width:40
	...
	name:'nombre_con'
	  ...
	  name:'est_new'
		...
		,align:'center'	
		,hidden:true
		},{name:'funcion',index:'funcion',align:'left', width:130,sortable:false,search:false	
	},{name:'imprime',index:'imprime',align:'left', width:120,sortable:false,search:false	
	}
	],
	...
	height:'auto',
	width:'auto',
    sortname: 'estado desc, servicio desc,fecha_elaboracion desc',
    sortorder: " ",
    viewrecords: true,
     caption: 'Planillas de Control',
/*   jsonReader: {
		...
		for(var i=0;i<ids.length;i++){
			var cl = ids[i];
			 ...
			 // var id_tarje= rowDatas['id_tarjeta'];
			  ...
			  var estado= rowDatas['servicio'];
			   ...
			   var id_tarj= rowDatas['id_tarjeta'];
			...
			//  if(est_ant != est_now){ 
			 ...
			 des = "<input  type='button' value='Descartar' "+estadop+" onclick=descarta_planilla("+i+");jQuery('#descarta_planilla').dialog('open'); />";
                         imp = "<form  action='../fpdf/imprime_planilla.php' method='post' target='_blank' id='fplanilla'><input type='hidden' value='"+id_plani+"' name='planilla' id='planilla' /><input type='hidden' value='"+id_tarj+"' name='id_tarj' id='id_tarj' /><input  type='submit' value='Imp'/></form>";
...
/*	nta = "<input style='height:22px;width:60px;' type='button' value='Ver Tarj' onclick=jQuery('#vertc').dialog('open');ver_tarjeta("+id_tarje+"); />";
	...
	//jQuery('#list10_d').jqGrid('setGridParam',{url:'subgrid.php?q=1&id=2',page:1}); jQuery('#list10_d').jqGrid('setCaption','Invoice Detail: 2') .trigger('reloadGrid')+"/>"; 
			...
			//se = "<a href='#' id='trigger'>TRIGGER</a>"; 
			//ce = "<input style='height:22px;width:20px;' type='button' value='C' onclick=jQuery('#crud').restoreRow("+cl+"); />"; 
			jQuery("#crudtc").setRowData(ids[i],{funcion:anu+idplanilla+numplani+des})
			jQuery("#crudtc").setRowData(ids[i],{imprime:imp})
		}	
	},
