...
<title>Tarjetas de control</title>
...
/*	},{
	  ...
	  name:'est_new'
		...
		,align:'center'	
		,hidden:true	
	},{name:'act',index:'act',align:'left', width:130,sortable:false,search:false}
	],
...
jQuery("#crudtc").jqGrid({ 
	 ...
	 datatype: "json", 
	  mtype: 'GET',
	  colNames:['Codigo','Tarjeta','Conductor','','','','','Movil','Vigencia','','Fecha Corte','Servicio','','','Grup','','Total P','Acciones','Doc Guia']
	  , colModel :[
	{name:'id_tarjeta'
		,index:'id_tarjeta'
		,width:55
...
/*	},{
	  ...
	  name:'grupo'
        ...
        },{name:'guia',index:'guia'//align:'center'
		,hidden:false,width:150,search:false}
	...
	height:'auto',
	width:'auto',
    sortname: 'a.id_tarjeta desc,a.estado,fecha_plazo_a',
    sortorder: "asc",
    viewrecords: true,
     caption: 'Tarjetas de Control permitidas',
/*   jsonReader: {
		repeatitems : true,
		cell:"",
		id: "0"
...
///////grilla para tarjetas suspendidas

//// detalle de tarjetas de control

jQuery("#crudtcsus").jqGrid({ 

     url:'gridtarjetasus.php?q=1',
	 datatype: "json", 
	  mtype: 'GET',
	  colNames:['Codigo','Tarjeta','Conductor','','','','','Movil','Vigencia','','Fecha Corte','Servicio','','','Grup','','Total P','Acciones','Doc Guia']
	  , colModel :[
...
/*	},{
	...
	height:'auto',
	width:'auto',
    sortname: 'a.estado,fecha_plazo_a',
    sortorder: "DESC",
    viewrecords: true,
     caption: 'Tarjetas de Control  Suspendidas',
/*   jsonReader: {
		repeatitems : true,
		cell:"",
		id: "0"
	...
	loadComplete:function(){
		...
		//setTimeout(function(){jQuery('#crudtcsus').jqGrid('setCaption','Detalle de tarjetas de control') .trigger('reloadGrid')},15000)
		var ids = jQuery("#crudtcsus").getDataIDs();
		for(var i=0;i<ids.length;i++){
			var cl = ids[i];
			 var rowDatas = jQuery(this).getRowData(cl);
			  var id_tarje= rowDatas['id_tarjeta'];
			  ...
			  if(aniof<anyo){
			   ...
			   jQuery("#crudtcsus").setRowData(rowDatas.id_tarjeta,{servicio:"<font color='blue'>Permitido</font>"}) 
			   }else{
			   var estado="disabled=disabled";
			   jQuery("#crudtcsus").setRowData(rowDatas.id_tarjeta,{servicio:"<font color='orange'>suspendido</font>"}) 
			   }
			//  }else{
//			   jQuery("#crudtcsus").setRowData(rowDatas.id_tarjeta,{servicio:"<font color='green'>Permitido</font>"}) 
			  }
			 ...
			 var estado="disabled=disabled";
			   jQuery("#crudtcsus").setRowData(rowDatas.id_tarjeta,{servicio:"<font color='red'>Suspendido</font>"}) 
			 }
	...
	//jQuery('#list10_d').jqGrid('setGridParam',{url:'subgrid.php?q=1&id=2',page:1}); jQuery('#list10_d').jqGrid('setCaption','Invoice Detail: 2') .trigger('reloadGrid')+"/>"; 
			...
			//se = "<a href='#' id='trigger'>TRIGGER</a>"; 
			//ce = "<input style='height:22px;width:20px;' type='button' value='C' onclick=jQuery('#crud').restoreRow("+cl+"); />"; 
			jQuery("#crudtcsus").setRowData(ids[i],{act:nta+ntarj+id_movi+plani+co+g})
			
		}	
	},
	    editurl: 'gridtarjetasus.php?q=1'// this is dummy existing url 
		
		 });
		  jQuery("#crudtcsus").jqGrid('navGrid','#pcrudtcsus',{edit:false,add:false,del:false,searchtext:'Busqueda', excel:true},{height:400,reloadAfterSubmit:true,closeAfterEdit : true},{closeOnEscape:true},{closeOnEscape:true},{sopt:['cn','bw','eq','ne','lt','gt','ew','le','ge']}).navButtonAdd('#pcrudtcsus',{
                                caption:"Exportar", 
                                buttonicon:"ui-icon-calculator", 
                                onClickButton: function(){ 
                                  exportExcelsus();
...
function exportExcelsus()
	...
	document.getElementById('frmsuspendidas').submit();
		   ...
		   jQuery("#crudtcsus").jqGrid('setGroupHeaders', { useColSpanStyle: true, groupHeaders:[ {startColumnName: 'nombre1', numberOfColumns: 2, titleText: '<em>Nombres</em>'}, {startColumnName: 'apellido1', numberOfColumns: 2, titleText: 'Apellidos'}, {startColumnName: 'tarj', numberOfColumns: 2, titleText: 'Tarjeta Ctrl'}	 ] });
//	 
  jQuery("#crudtcsus").jqGrid('filterToolbar');  
...
////alertas modales 											
jQuery('#vertc').dialog({autoOpen: false, modal:true,width:710,height:900,}); 
...
<tr><td>&nbsp;</td></tr></table><div id="pcrudnov" align="center"> </div>
  <p>&nbsp;</p>
  <span class="Estilo1">Tarjetas de Control Permitidas </span>
...
<div id="grabaplanilla" ></div>
 ...
 <p>&nbsp;</p>
  <span class="Estilo1">Tarjetas de Control Suspendidas</span>
<table align="center" id="crudtcsus"  >
<tr><td>&nbsp;</td></tr></table><div id="pcrudtcsus" align="center"></div>

<form method="post" action="excelpermitidastaxi.php" id="frmpermitidas" name="frmpermitidas">
    
</form>

<form method="post" action="excelsuspendidastaxi.php" id="frmsuspendidas" name="frmsuspendidas">
