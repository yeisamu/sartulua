...
//$dbConfig['dbDatabase'] = 'sart'; 		/* Database */
					/* id column */
...
$crudColumns1 =  array(
	...
	,'codigo'=>'codigo'
	,'documento'=>'documento'
	,'eps'=>'entidad_salud.eps'
	,'id_eps'=>'entidad_salud.id_eps'
...
foreach ($crudColumns2 as $key => $value){ 
	if(isset($_REQUEST[$key])){
		$crudColumnValues2[$key] = '"'.fnCleanInputVar($_REQUEST[$key]).'"';
	}
}
...
switch($postConfig['action']){
	case $crudConfig['read']:
		...
		/*query to count rows*/
		$sql='select count('.$postConfig['id'].') as numRows from (con_doc inner join conductor on con_doc.id_conductor=conductor.id_conductor) inner join documento on con_doc.id_doc=documento.id_doc ';
		...
		/*Run the data query*/
		$sql = 'select '.implode(',',$crudColumns1).'  from ((con_doc left join entidad_salud on con_doc.id_eps=entidad_salud.id_eps) inner join conductor on con_doc.id_conductor=conductor.id_conductor) inner join documento on con_doc.id_doc=documento.id_doc';
