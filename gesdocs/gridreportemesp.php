...
$consulta="select vehiculo.id_movil";
for($mes=1; $mes<=12;$mes++){
   
    $consulta = $consulta.", sum(if(month(fecha)=".$mes.",1,0)) as '".$meses[$mes - 1]."'"; 
}
$consulta = $consulta.", count(fecha) as total,vehiculo.grupo from reporte_planilla right join vehiculo on reporte_planilla.id_movil=vehiculo.id_movil where year(fecha)= ".$ano;
return $consulta;
}
...
// $var="SELECT COUNT(*) AS count from servicio where 1 ".$wh.$wd;
   $result = mysql_query("SELECT COUNT(*) AS count from reporte_planilla right join vehiculo on reporte_planilla.id_movil=vehiculo.id_movil where year(fecha)='$ano' ".$wh.$wd);
 ...
 //$row['count'];
        ...
        if ($start<0) $start = 0;
  $SQL = consulta_ano($anio).$wd." group by vehiculo.id_movil order by ".$sidx." ". $sord;
 // "SELECT distinct(year(`fecha_reg`)) as anio FROM `servicio`  ORDER BY ".$sidx." ". $sord." LIMIT ".$start." , ".$limit; 
	...
	$responce->rows[$i]['cell']=array(utf8_encode($row[id_movil]),$anio,$row[ENERO],$row[FEBRERO],$row[MARZO],$row[ABRIL],$row[MAYO],$row[JUNIO],$row[JULIO],$row[AGOSTO],$row[SEPTIEMBRE],$row[OCTUBRE],$row[NOVIEMBRE],$row[DICIEMBRE],$row[total],$row[grupo]); $i++; }
	//echo $SQL;
