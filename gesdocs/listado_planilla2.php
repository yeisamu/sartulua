...
for($i=$in;$i<=$fin;$i++){
	...
	//echo  $con="update reporte_planilla set `fecha`=$fecha_elab, `id_planilla`=$id_panilla,`id_movil`=$id_movil, `id_conductor`= $id_conductor, `codigo`=$codigo, `nombre_con`=$nombre, `destino`=$ciudad_d, `elab`=$usr_elab, `recibido`=$usr_rec, `liquidado`=$liquidado where n_planilla=$i ";
	 $veri=mysql_query("select * from planilla inner join (tarjeta_control inner join conductor on tarjeta_control.id_conductor=conductor.id_conductor) on planilla.id_tarjeta=tarjeta_control.id_tarjeta where n_planilla=$i");
	 $fila=mysql_fetch_array($veri);
	  ...
	  $tot_registro=mysql_num_rows($veri);
	   if($tot_registro>0){	   
	  $dato=mysql_query("insert into reporte_planilla (`fecha`, `id_planilla`, `n_planilla`, `id_movil`, `grupo`, `id_conductor`, `codigo`, `nombre_con`, `destino`, `elab`, `recibido`, `estado`,observaciones, `liquidado`)values ('$fecha_elab',$id_panilla,$i,'$id_movil','$id_grupo',$id_conductor,'$codigo','$nombre','$ciudad_d','$usr_elab', '$usr_rec',$estado,'$obs',$liquidado) ");
	  }else{
...
?>
<span class="Estilo1">Planillas  <?php echo $n_grupo?></span>
