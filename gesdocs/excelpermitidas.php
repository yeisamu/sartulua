...
header("content-disposition: attachment;filename=reporte_simit_permitidas.xls");
...
INNER JOIN conductor b ON a.`id_conductor` = b.`id_conductor` WHERE a.estado = 1 and if(`fecha_plazo_a`<DATE_FORMAT(now(),'%Y/%m/%d %H:%i'),'Suspendido','Permitido')= 'Permitido' )  order by estado,b.id_conductor")
?>
...
$filamovil=mysql_fetch_array($selmov);
  ...
  <tr>
    ...
    <td class="ui-corner-all <?php echo $prop ?>"><?php echo $filamovil['id_movil'] ?></td>
    <td class="ui-corner-all <?php echo $prop ?>"><?php echo $filamovil['placa'] ?></td>
    <td class="ui-corner-all <?php echo $prop ?>"><?php echo $filamovil['grupo'] ?></td>
  </tr>
