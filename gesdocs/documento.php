...
jQuery("#crudnov").jqGrid({ 
    ...
    sortorder: "desc",
    viewrecords: true,
     caption: 'Pago de Diarios y polizas',
...
//alert(id);
	...
	loadComplete:function(){
	    ...
	    editurl: 'gridreportediario.php?q=1'// this is dummy existing url 
		  ...
		  jQuery("#crudnov").jqGrid('navGrid','#pcrudnov',{edit:false,add:false,del:false,search:false,searchtext:'Busqueda'},{height:400,reloadAfterSubmit:true,closeAfterEdit : true},{closeOnEscape:true},{closeOnEscape:true},{sopt:['cn','bw','eq','ne','lt','gt','ew','le','ge']});
...
//		});} }
	...
	name:'apellido2'
	 ...
	 ,{name:'parte',index:'parte',align:'center', width:80,sortable:false,search:false}
	  ,{name:'tarj',index:'tarj',align:'center', width:60,sortable:false,search:false},
	  {name:'ntarj',index:'ntarj',align:'left', width:120,sortable:false,search:false},
	],
...
var mygrid1 =jQuery("#crudtc").jqGrid({ 
	...
	name:'tarjeta'
	  ...
	  name:'estado'
		...
		name:'oper',index:'oper',align:'center', width:410,sortable:false,search:false
	...
	imp = "<input  type='button' value='Imprimir' onclick=imp("+i+"); />";
	...
	//plani = "<input  type='button' value='Planilla' "+estado+" onclick=abre_planilla("+i+","+control_p+");jQuery('#planillas').slideToggle();jQuery('#planillas').dialog('open') />";
			...
			jQuery("#crudtc").setRowData(ids[i],{oper:be+imp+fe+co+ver+cer+ntarj+condoc+autor})
...
jQuery('#actualizatc').dialog({autoOpen: false, modal:true,width:700,height:800,});
...
jQuery("#crudsim").jqGrid({ 
	...
	name:'valor'
		...
		function(elm){setTimeout(function(){
			...
			sliderAcessArgs: {touchonly: true }
			});
	...
	/* onSelectRow:
		...
		if(ids && ids!==lastsel2){ 
		jQuery('#crudsim').restoreRow(lastsel2);
		 jQuery('#crudsim').editRow(ids,true);
		  lastsel2=ids; }
		  },*/
		//jQuery('#crud').editRow("+cl+"); 
	///agregar botones de accion
	...
	anulaparte = "<input style='height:22px;' type='button' value='Cerrar' onclick=anulamulta("+idsimi+");jQuery('#cierramultas').dialog('open') />";
		...
		// var verdadero= true
jQuery("#crudsim").jqGrid('navGrid','#pcrudsim',{edit:false,add:false,del:true,search:false},{height:400,reloadAfterSubmit:true,closeAfterEdit : true},{closeOnEscape:true},{closeOnEscape:true},{sopt:['cn','bw','eq','ne','lt','gt','ew','le','ge']});
////		 
...
<!-- <div id="Layer2"><span class="Estilo2">Diarios y polizas</span>
