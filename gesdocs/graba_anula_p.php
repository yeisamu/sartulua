...
if(isset($_REQUEST['loginan']) && !empty($_REQUEST['loginan']) && isset($_REQUEST['passan']) && !empty($_REQUEST['passan'])){
	...
	//check for the valid username and password
      $consulta = "select * from acc_usuario where login='$username' and clave='$con' and admin=2 or admin=1";
...
$actutarje="update reporte_planilla set estado=3,observaciones ='$obs',recibido='$login' where id_planilla=$id_planilla";
$querytarje=mysql_query($actutarje);

	if(!$sqltar || !$querytar || !$querytarje){
	echo $modal="
